...
#Script to install Splunk Forwarder on Fedora
...
DOWNLOAD_URL="https://download.splunk.com/products/universalforwarder/releases/10.0.2/linux/splunkforwarder-10.0.2-e2d18b4767e9.x86_64.rpm"
RPM_PKG_NAME="splunkforwarder-10.0.2-e2d18b4767e9.x86_64.rpm"
...
#Download forwarder
if [ ! -f "/tmp/$RPM_PKG_NAME" ]; then
  echo "Downloading Splunk Universal Forwarder..."
  curl -fsSL -o "/tmp/$RPM_PKG_NAME" "$DOWNLOAD_URL"
fi

#Install forwarder
echo "Installing Splunk Universal Forwarder..."
if command -v dnf >/dev/null 2>&1; then
  dnf install -y "/tmp/$RPM_PKG_NAME"
else
  rpm -ivh "/tmp/$RPM_PKG_NAME" --replacepkgs
fi
